...
(($, { behaviors }, { testMessages }) => {
  ...
  behaviors.js_message_test = {
    attach() {
      $(once('messages-details', '[data-drupal-messages-area]'))
      ...
      $(once('add-multiple', '[data-action="add-multiple"]'))
        .on('click', () => {
          /**
      ...
      $(once('remove-multiple', '[data-action="remove-multiple"]'))
        .on('click', () => {
      ...
      $(once('add-multiple-error', '[data-action="add-multiple-error"]'))
        .on('click', () => {
      ...
      $(once('remove-type', '[data-action="remove-type"]'))
        .on('click', () => {
          Array.prototype.map
            .call(
      ...
      $(once('clear-all', '[data-action="clear-all"]'))
        .on('click', () => {
          messageObjects.default.zone.clear();
        });
      $(once('id-no-status', '[data-action="id-no-status"]'))
        .on('click', () => {
