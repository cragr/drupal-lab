...
class TokenReplaceKernelTest extends TokenReplaceKernelTestBase {
  ...
  public function testSystemSiteTokenReplacement() {
    ...
    $this->assertEquals((new BubbleableMetadata())->addCacheContexts(['url.site']), $bubbleable_metadata);
    // Test [site:base-url] and [site:base-path] token with "index.php" in the
    // path.
    $request = Request::create('/index.php/');
    $request->server->add($server);
    $request_stack->push($request);
    $this->assertEquals('http://localhost/subdir', $this->tokenService->replace('[site:base-url]'));
    $this->assertEquals('/subdir', $this->tokenService->replace('[site:base-path]'));
  }
