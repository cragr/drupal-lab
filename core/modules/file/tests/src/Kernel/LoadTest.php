...
class LoadTest extends FileManagedUnitTestBase {
  ...
  public function testSingleValues() {
    ...
    $this->assertEquals($file->id(), $by_fid_file->id(), 'Loading by fid got the same fid.', 'File');
    $this->assertEquals($file->getFileUri(), $by_fid_file->getFileUri(), 'Loading by fid got the correct filepath.', 'File');
    $this->assertEquals($file->getFilename(), $by_fid_file->getFilename(), 'Loading by fid got the correct filename.', 'File');
    $this->assertEquals($file->getMimeType(), $by_fid_file->getMimeType(), 'Loading by fid got the correct MIME type.', 'File');
    $this->assertEquals($file->isPermanent(), $by_fid_file->isPermanent(), 'Loading by fid got the correct status.', 'File');
  ...
  public function testMultiple() {
    ...
    $this->assertEquals($file->id(), $by_path_file->id(), 'Loading by filepath got the correct fid.', 'File');
  ...
  public function testUuidValues() {
    ...
    $this->assertEquals($file->id(), $by_uuid_file->id(), 'Loading by UUID got the same fid.', 'File');
