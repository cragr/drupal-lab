  ...
  public function testAccess() {
    ...
    foreach ($views_data['access_callback'] as $type => $info) {
      if (!in_array($type, ['title', 'help'])) {
        ...
        $this->assertArrayNotHasKey('access_callback_arguments', $view->field, 'Make sure the user got no access to the access_callback_arguments field ');
      }
    }
    ...
    foreach ($views_data['access_callback'] as $type => $info) {
      if (!in_array($type, ['title', 'help'])) {
        $this->assertArrayNotHasKey('access_callback', $view->field, 'Make sure the user got no access to the access_callback field ');
