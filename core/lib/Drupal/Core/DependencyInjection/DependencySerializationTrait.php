...
trait DependencySerializationTrait {
  ...
  public function __sleep() {
    ...
    foreach ($vars as $key => $value) {
      ...
      elseif ($value instanceof EntityStorageInterface) {
        ...
        $this->entityStorages[$key] = $value->getEntityTypeId();
        unset($vars[$key]);
      }
    }
  ...
  public function __wakeup() {
    ...
    // case, too.
    if ($this->entityStorages && (!$phpunit_bootstrap || $container->has('entity_type.manager'))) {
      /** @var \Drupal\Core\Entity\EntityTypeManagerInterface $entity_type_manager */
      $entity_type_manager = $container->get('entity_type.manager');
      foreach ($this->entityStorages as $key => $entity_type_id) {
