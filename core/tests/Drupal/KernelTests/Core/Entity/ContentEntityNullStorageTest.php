...
class ContentEntityNullStorageTest extends KernelTestBase {
  ...
  public function testEntityQuery() {
    $this->assertSame(0, \Drupal::entityQuery('contact_message')->accessCheck(TRUE)->count()->execute(), 'Counting a null storage returns 0.');
    $this->assertSame([], \Drupal::entityQuery('contact_message')->accessCheck(TRUE)->execute(), 'Querying a null storage returns an empty array.');
    $this->assertSame([], \Drupal::entityQuery('contact_message')->accessCheck(TRUE)->condition('contact_form', 'test')->execute(), 'Querying a null storage returns an empty array and conditions are ignored.');
    $this->assertSame([], \Drupal::entityQueryAggregate('contact_message')->accessCheck(TRUE)->aggregate('name', 'AVG')->execute(), 'Aggregate querying a null storage returns an empty array');
