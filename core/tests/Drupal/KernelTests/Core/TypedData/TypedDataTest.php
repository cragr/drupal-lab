...
class TypedDataTest extends KernelTestBase {
  ...
  public function testGetAndSet() {
    ...
    $this->assertEqual('2014-01-02T20:00:00+11:00', $typed_data->getValue());
    $typed_data->setValue(NULL);
    $this->assertNull($typed_data->getDateTime());
    ...
    $this->assertEqual(40, $typed_data->getDuration()->d, 'DurationIso8601 value was changed and set by duration string.');
    $this->assertIsString($typed_data->getString());
    $this->assertEqual(0, $typed_data->validate()->count());
    $typed_data->setValue(NULL);
    ...
    $this->assertEqual('P0Y0M40DT0H0M0S', $typed_data->getValue());
    $typed_data->setValue(NULL);
    ...
    $this->assertEqual(14400, $typed_data->getDuration()->s, 'Time span was changed');
    $this->assertIsString($typed_data->getString());
    $this->assertEqual(0, $typed_data->validate()->count());
    $typed_data->setValue(NULL);
    ...
    $this->assertEqual(60 * 60 * 4, $typed_data->getValue());
    $typed_data->setValue(NULL);
    $this->assertNull($typed_data->getDuration());
    ...
    $this->assertEqual(fgets(fopen($files[1]->getFileUri(), 'r')), fgets($typed_data->getValue()), 'Binary value was changed.');
    $this->assertIsString($typed_data->getString());
    $this->assertEqual(0, $typed_data->validate()->count());
  ...
  public function testTypedDataLists() {
    ...
    $this->assertEqual($value, $typed_data->getValue(), 'List value has been set.');
    // Test iterating.
    $count = 0;
    ...
    $this->assertEqual('one, two, three', $typed_data->getString());
    $typed_data[1] = '';
    $this->assertEqual('one, three', $typed_data->getString());
    ...
    $this->assertEqual([NULL, '', 'three'], $typed_data->getValue());
    // Test unsetting.
    unset($typed_data[1]);
    $this->assertCount(2, $typed_data);
    // Check that items were shifted.
    $this->assertEqual('three', $typed_data[1]->getValue());
    ...
    $this->assertEqual(1, $typed_data->count());
  ...
  public function testTypedDataListsFilter() {
    ...
    $this->assertEqual(2, $typed_data->count());
    $this->assertEqual('zero', $typed_data[0]->getValue());
    $this->assertEqual(0, $typed_data[0]->getName());
    $this->assertEqual('one', $typed_data[1]->getValue());
    $this->assertEqual(1, $typed_data[1]->getName());
    ...
    $this->assertEqual(0, $typed_data->count());
    ...
    $this->assertEqual('zero', $typed_data[0]->getValue());
    $this->assertEqual(0, $typed_data[0]->getName());
    $this->assertEqual('two', $typed_data[1]->getValue());
    $this->assertEqual(1, $typed_data[1]->getName());
  }
  ...
  public function testTypedDataMaps() {
    ...
    $this->assertEqual(array_keys($value), array_keys($typed_data->getDataDefinition()->getPropertyDefinitions()));
    $definition = $typed_data->getDataDefinition()->getPropertyDefinition('one');
    $this->assertEqual('string', $definition->getDataType());
    $this->assertNull($typed_data->getDataDefinition()->getPropertyDefinition('invalid'));

    // Test getting and setting properties.
    $this->assertEqual('eins', $typed_data->get('one')->getValue());
    $this->assertEqual($value, $typed_data->toArray());
    $typed_data->set('one', 'uno');
    $this->assertEqual('uno', $typed_data->get('one')->getValue());
    // Make sure the update is reflected in the value of the map also.
    $value = $typed_data->getValue();
    $this->assertEqual(['one' => 'uno', 'two' => 'zwei', 'three' => 'drei'], $value);

    $properties = $typed_data->getProperties();
    $this->assertEqual(array_keys($value), array_keys($properties));
    ...
    $this->assertEqual(['one', 'two', 'three'], array_keys($typed_data->getProperties()));
    $this->assertEqual(['foo', 'one', 'two', 'three'], array_keys($typed_data->getValue()));

    // Test getting the string representation.
    $typed_data->setValue(['one' => 'eins', 'two' => '', 'three' => 'drei']);
    $this->assertEqual('eins, drei', $typed_data->getString());
    ...
    $this->assertEqual('null', $typed_data->get('zero')->getValue());
    $definition = $typed_data->get('zero')->getDataDefinition();
    $this->assertEqual('any', $definition->getDataType(), 'Definition for a new map entry returned.');
  }
  ...
  public function testTypedDataValidation() {
    ...
    $this->assertEqual($message, $violations[0]->getMessage(), 'Translated violation message retrieved.');
    ...
    $this->assertEqual(2, $violations->count());
    ...
    $violations = $field_item->validate();
    $this->assertEqual(1, $violations->count());
    $this->assertEqual('0.value', $violations[0]->getPropertyPath());
    ...
    $this->assertEqual('string', $violations[0]->getInvalidValue());
